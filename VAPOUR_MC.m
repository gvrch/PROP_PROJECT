...
Apipe_std = (sqrt(Apipe/pi)+0.001*1e-3)^2*pi-Apipe;
Apipe_avg = Apipe;
Av_std = (sqrt(Av/pi)+0.001*1e-3)^2*pi-Av;
Av_avg = Av;
Ainj_std = (sqrt(Ainj/pi)+0.001*1e-3)^2*pi-Ainj;
Ainj_avg = Ainj;
At_std = (sqrt(At/pi)+0.001*1e-3)^2*pi-At;
At_avg = At;
Ae_std = (sqrt(Ae/pi)+0.001*1e-3)^2*pi-Ae;
Ae_avg = Ae;
Q_dot_std = Q_dot*2/100;
Q_dot_avg = Q_dot;
