...
mass    = 4;            % S/C mass                                              [kg]
Q_design = linspace(10, 13, 100);   % Only for design
N = 2000;              % N MONTECARLO

k_pipe = 1;        % Coefficiente di perdita tubi
k_inj = 1/Cd^2;

r_pipe = 250e-6;      % Area del tubo di alimentazione (m^2) CAMBIARE
Apipe = r_pipe^2*pi;
...
for iii = 1:length(Q_design)
    ...
    Ainj_std = (sqrt(Ainj/pi)+(1e-6)/3)^2*pi-Ainj;
    Ainj_avg = Ainj;
    At_std = (sqrt(At/pi)+(1e-6)/3)^2*pi-At;
    At_avg = At;
    ...
    K_tot_val = 0.5/rho_l.*(k_inj./Ainj_val.^2 + k_pipe./Apipe.^2 + k_v./Av.^2);
    % dP_tot = K_tot*m_dot^2;
